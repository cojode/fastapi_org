...
Для доступа к эндпоинтам составьте API-ключ в `.env` и укажите его в заголовке "X-API-Key":

```bash
FASTAPI_ORG_API_KEY=mysecretapikey123
```

Подробнее про эндпоинты и удобное подключение API-ключа в документациях: 
